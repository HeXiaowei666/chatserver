...
vector<Group> GroupModel::queryGroups(int userid)
{
   ...
   // 1. 组装sql语句
    char sql[1024] = {0};
    sprintf(sql, "select a.id, a.groupname, a.groudesc from allgroup a inner join \
...
vector<int> GroupModel::queryGroupUsers(int userid, int groupid)
{
    // 1. 组装sql语句
    char sql[1024] = {0};
    sprintf(sql, "select userid from groupuser where userid = %d and userid != %d", groupid, userid);
